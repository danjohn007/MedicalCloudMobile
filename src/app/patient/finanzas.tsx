...
export default function FinanzasScreen() {
  ...
  const total = useMemo(() => data?.payments?.reduce((acc, p) => acc + (p.amount || 0), 0) || 0, [data]);
        ...
        >
          ...
          <View style={s.summaryWrap}>
            <View style={s.statCard}>
              ...
              <Text style={s.statValue}>${(data?.summary?.this_month || 0).toFixed(2)}</Text>
            </View>
            <View style={s.statCard}>
              <Text style={s.statLabel}>Este año</Text>
              <Text style={s.statValue}>${(data?.summary?.this_year || 0).toFixed(2)}</Text>
            </View>
            <View style={s.statCard}>
            ...
            {(data?.payments || []).map((p) => (
              <View key={p.id} style={s.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={s.meta}>{(p.method || 'metodo').toUpperCase()} · {String(p.status || 'desconocido').toUpperCase()}</Text>
                  {p.created_at ? <Text style={s.meta}>{String(p.created_at).slice(0, 16).replace('T', ' ')}</Text> : null}
                </View>
                <Text style={s.amount}>${(p.amount || 0).toFixed(2)} {p.currency || 'MXN'}</Text>
              </View>
            ))}
          </View>
...
const s = StyleSheet.create({
  ...
  statCard: { flex: 1, borderWidth: 1, borderColor: MC.border, borderRadius: 12, padding: 10, backgroundColor: MC.background },
  statLabel: { fontSize: 11, color: MC.textMuted, marginBottom: 4 },
  statValue: { fontSize: 15, fontWeight: '700', color: MC.textPrimary },
  row: { borderWidth: 1, borderColor: MC.border, borderRadius: 12, padding: 12, flexDirection: 'row', gap: 10, alignItems: 'center' },
