...
const normStatus = (s: string): StatusKey =>
  s in STATUS_META ? (s as StatusKey) : "pending";
...
export default function CitasScreen() {
  ...
  const counts: Record<FilterKey, number> = {
    ...
    pending: items.filter((a) =>
      ["pending", "pending_payment"].includes(normStatus(a.status)),
    ).length,
    confirmed: items.filter((a) => normStatus(a.status) === "confirmed").length,
    in_consultation: items.filter(
      (a) => normStatus(a.status) === "in_consultation",
    ).length,
  ...
  const filtered = items.filter((a) => {
    ...
    if (filter === "pending")
      return ["pending", "pending_payment"].includes(normStatus(a.status));
    if (filter === "no_show")
      return ["no_show", "missed"].includes(normStatus(a.status));
      ...
      { text: "No", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            setSelected(null);
            load();
            Alert.alert("Cita cancelada", "Tu cita ha sido cancelada.");
          } catch (e: any) {
      ...
      <View style={s.fixedTop}>
        ...
        >
          <StatChip
            ...
            icon="calendar"
          />
          {tab === "upcoming" && counts.confirmed > 0 && (
            <StatChip
              label="Confirmadas"
          ...
          {tab === "upcoming" && counts.in_consultation > 0 && (
            <StatChip
              label="En consulta"
            ...
            return (
              ...
              >
                ...
                <Text
                  style={[
                    s.filterText,
                    active && { color: f.color, fontWeight: "700" },
                  ]}
                >
                  {f.label}
                  {cnt > 0 ? ` (${cnt})` : ""}
      ...
      {loading ? (
        ...
        <View style={s.center}>
          <View style={s.emptyCircle}>
            <Icon name="warning" size={42} color={MC.error} />
          </View>
        ...
        <View style={s.center}>
          <View style={s.emptyCircle}>
            <Icon name="calendar" size={42} color={MC.textMuted} />
          </View>
    ...
    <Pressable onPress={onPress} style={s.card}>
      <View style={s.cardDateBlock}>
        ...
        <Text style={s.cardMonth}>
          {dt.toLocaleString("es-MX", { month: "short" }).toUpperCase()}
        </Text>
        <Text style={s.cardTime}>
          {formatTime12(dt.toTimeString().slice(0, 5))}
        </Text>
      </View>
      <View style={s.cardBody}>
        ...
        {appt.specialty ? (
          <Text style={s.cardSpecialty}>{appt.specialty}</Text>
        ) : null}
        <View style={s.cardFooter}>
          {appt.fee > 0 ? (
            ...
            <View style={s.cardFee}>
              ...
              <Text style={[s.cardFeeText, { color: MC.success }]}>
                Sin costo
              </Text>
            </View>
          )}
...
function AppointmentDetail(props: {
  ...
  const dt = new Date(appt.scheduled_at);
  const dateLong = dt.toLocaleDateString("es-MX", {
    weekday: "long",
    day: "numeric",
    month: "long",
    year: "numeric",
      ...
      <View style={s.modalSheet}>
        ...
        <View style={s.modalHeader}>
          <View style={{ flex: 1 }}>
            ...
            {appt.specialty ? (
              <Text style={s.modalSubtitle}>{appt.specialty}</Text>
            ) : null}
          </View>
        ...
        >
          <InfoRow
            ...
            iconBg={MC.primaryLight}
            label="Fecha y hora"
            value={`${dateLong} - ${timeLong}`}
          />
          <InfoRow
            icon="user"
            iconColor="#8B5CF6"
      ...
      <View style={{ flex: 1 }}>
        ...
        {props.subtitle ? (
          <Text
            style={[
              s.infoSubtitle,
              props.subtitleColor ? { color: props.subtitleColor } : null,
            ]}
          >
            {props.subtitle}
          </Text>
