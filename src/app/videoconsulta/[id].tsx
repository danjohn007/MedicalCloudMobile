...
function formatDateRange(startAt: Date, endAt: Date) {
  ...
  const start = startAt.toLocaleTimeString("es-MX", {
    hour: "2-digit",
    minute: "2-digit",
  });
  const end = endAt.toLocaleTimeString("es-MX", {
    hour: "2-digit",
    minute: "2-digit",
  });
...
export default function VideoconsultaScreen() {
  ...
  const { id, scheduled_at, status, doctor_name, duration } =
  ...
  const endAt = useMemo(() => {
    ...
    const minutes =
      minsFromApi || (Number.isFinite(minsFromParams) ? minsFromParams : 45);
        ...
        {loading ? (
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.countdown}>
              Abre en {formatClock(countdownToOpen)}
            </Text>
          </View>
        )}
